...
当前实现采用"规则优先 + LLM 按需补充"策略（详见《智能问答逻辑详解》）：

- **规则判断**（始终执行）：关键词匹配、追问检测、路由判断
- **LLM 调用**（按需执行）：一次调用完成路由+追问+改写+扩展，返回 11 字段 JSON
- 规则不确定或需要 enrichment 时才调用 LLM
- LLM 默认使用 Ollama 本地小模型，降级到 MiniMax
...
- 追问线索词检测
- 超范围线索检测
- 路由判断（kb_qa / out_of_scope）

强关键词命中时直接确定路由，无需调用模型。

### 5.2 LLM 调用（按需执行）

规则不确定或需要 enrichment 时，调用 LLM 一次完成：

- 路由判断（kb_qa / out_of_scope）
- 追问检测
- 上下文依赖判断
- 改写 query（rewrite_query）
- 关键词抽取（keywords_hit）
- 实体抽取（entities）
- 检索扩展（search_terms / search_queries）

LLM 默认使用 Ollama 本地小模型（gemma4:e4b），失败时降级到 MiniMax。
...
- 置信度：取两者中更高的
- 关键词/实体/检索词：两边合并去重
- rewrite_query：经稳定化处理，防止模型丢关键词

### 5.4 已知问题与改进方向

当前一次 LLM 调用同时完成分类和生成，4B 小模型对 11 字段 JSON 遵从度不足。改进方案见《问题理解分层方案设计》。
...
当前改写策略：
- 规则改写：追问时拼接上一轮话题（`"{上一轮问题}；补充追问：{当前问题}"`）
- LLM 改写：生成 rewrite_query，补全上下文但不改掉用户原意
- 稳定化：如果 LLM 改写丢掉了规则命中的关键词，回退到规则改写
...
- `QuestionAnalyzer`：规则判断、小模型分类（路由 + 追问 + 上下文依赖）
- `QueryExpander`：大模型检索扩展（rewrite_query / search_terms / search_queries）
